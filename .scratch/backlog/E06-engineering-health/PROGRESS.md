...
Worked on at length during E04 without being solved. The full evidence lives in the story; the
short version, so nobody restarts from zero:
...
absorb one. That is not a fix and the story stays open - its first criterion is that the cause be
identified - but the reproduction is now the hard part, and those three configurations will not
give you one.
