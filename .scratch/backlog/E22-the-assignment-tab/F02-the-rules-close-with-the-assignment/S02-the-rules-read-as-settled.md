...
- [x] After `assignment`, priority rules and assignment options are shown but cannot be edited, and a line says why.
- [x] Up to and including `assignment`, they are editable.
- [x] The phase comes from the market the screen holds, so a transition from the rail changes it at once, without leaving the tab.
- [x] Pinned by an e2e spec that walks a market from `assignment` to `market_days` with the tab open.
